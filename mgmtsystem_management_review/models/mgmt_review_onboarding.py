...
class ResCompany(models.Model):
    ...
    def action_open_mgmt_review_plan(self, action_ref=None):
        if not action_ref:
            action_ref = 'mgmtsystem_management_review.action_mgmt_review_plan_configurator'
        return self.env.ref(action_ref).read()[0]

    @api.model
    def action_open_mgmt_review_review(self, action_ref=None):
        if not action_ref:
            action_ref = 'mgmtsystem_management_review.action_mgmt_review_review_configurator'
        return self.env.ref(action_ref).read()[0]

    @api.model
    def action_open_mgmt_review_record(self, action_ref=None):
        if not action_ref:
            action_ref = 'mgmtsystem_management_review.action_mgmt_review_record_configurator'
        return self.env.ref(action_ref).read()[0]

    @api.model
    def action_close_mgmt_review_onboarding(self):
        """ Mark the invoice onboarding panel as closed. """
        self.env.company.mgmt_review_onboarding_state = 'closed'

    def get_and_update_mgmt_review_onboarding_state(self):
        ...
        return self.sudo().get_and_update_onbarding_state('mgmt_review_onboarding_state', steps)


class MgmtReviewPlan(models.Model):
    _inherit = 'management.review.plan'

    def action_save_onboarding_mgmt_review_plan_step(self):
...
class MgmtReviewReview(models.Model):
    ...
    def action_save_onboarding_mgmt_review_review_step(self):
        """ Set the onboarding step as done """
        pass
...
class MgmtReviewRecord(models.Model):
    ...
    def action_save_onboarding_mgmt_review_record_step(self):
        """ Set the onboarding step as done """
        pass
