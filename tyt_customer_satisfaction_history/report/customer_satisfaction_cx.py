...
class SurveyReport(models.AbstractModel):
    _name = 'report.tyt_customer_satisfaction_history.report_tyt_sscx'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet = workbook.add_worksheet('NPS por Sitio',)
        ...
        sheet.merge_range(
            'D2:L6', 'Encuesta de Satisfacción CX', format26_c_bold)
        ...
        def create_location_table(sheet, current_row):
            ...
            sheet.write(current_row, 1, 'Sitio', format21_gray_bold_sb)
            ...
            for location in locations:
                ...
                location_customers = tyt_satisfaction_survey_ids.filtered(
                    lambda x: x.location == location
                )
                
                total_location_customers = len(location_customers)
                ...
                category_averages = {
                    'cat_1': category_totals['cat_1'] / 3 / total_location_customers if total_location_customers > 0 else 0,
                    'cat_2': category_totals['cat_2'] / 2 / total_location_customers if total_location_customers > 0 else 0,
                    'cat_3': category_totals['cat_3'] / 3 / total_location_customers if total_location_customers > 0 else 0,
                    'cat_4': category_totals['cat_4'] / 2 / total_location_customers if total_location_customers > 0 else 0,
                    'cat_5': category_totals['cat_5'] / 3 / total_location_customers if total_location_customers > 0 else 0,
                    'cat_6': category_totals['cat_6'] / 2 / total_location_customers if total_location_customers > 0 else 0,
                    'cat_7': category_totals['cat_7'] / 2 / total_location_customers if total_location_customers > 0 else 0,
                }
        ...
        national_total = create_location_table(sheet, current_row)
        
        national_average = national_total / len(locations)
        ...
        sheet.write(current_row, 2, 'Operaciones', format21_gray_bold_sb)
        sheet.write(current_row, 3, 'Calidad', format21_gray_bold_sb)
        sheet.write(current_row, 4, 'Reclutamiento', format21_gray_bold_sb)
        ...
        sheet.merge_range(current_row, 10, current_row + 1, 10, 'PROMEDIO NACIONAL', format21_gray_bold_c)
        sheet.merge_range(current_row, 11, current_row + 1, 11, national_average, format21_gray_bold_c)

        sheet2 = workbook.add_worksheet("Satisfaccion_clientes - General")
        ...
        for i, survey in enumerate(tyt_satisfaction_survey_ids, 1):
            ...
            sheet2.write(current_row, 3, survey.location, format21_left)
